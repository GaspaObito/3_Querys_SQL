...
/****** Object:  View [dbo].[V_Int_MedPag]    Script Date: 6/05/2026 9:24:30 a. m. ******/
SET ANSI_NULLS ON
GO
...
ALTER View  [dbo].[V_Int_MedPag_corregido_vr] as
SELECT 
    ...
    THEN 
        ...
        ISNULL((
            ...
            WHERE sub.DCTO = MvCuadre.DCTO 
              ...
              AND (
                    -- CASO 1A: existen 110505 + 111005 → SOLO aplica a 110505
                    (
                        EXISTS (
                            SELECT 1
                            FROM MvCuadre x
                    ...
                    -- CASO 1B: existen 110505 + 13809501 → SOLO aplica a 110505
                    (
                        EXISTS (
                            SELECT 1
                            FROM MvCuadre x
                        ...
                        AND EXISTS (
                            ...
                            INNER JOIN MtMedPag mp ON mp.CODIGOCTA = x.BANCO
                            WHERE x.DCTO = MvCuadre.DCTO
                              AND mp.CODIGOCTA LIKE '%13809501%' 
                        )
                        AND MtMedPag.CODIGOCTA LIKE '%110505%'
                    )
                    ...
                    -- CASO 2: NO existen combinaciones → aplica normal
                    NOT (
                        (
                            EXISTS (
                                SELECT 1
