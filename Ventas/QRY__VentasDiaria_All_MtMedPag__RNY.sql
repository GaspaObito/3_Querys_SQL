...
SELECT        
	...
	MV.EFECTIVO_PV,MV.TARJETA_DEBITO_PV,TARJETA_CREDITO_PV,MV.BONOS_PROMO_200,MV.VENTAS_CREDITO_RNY,MV.TARJETA_KOMIDA_RNY,MV.CAMBIOS_DEVOLUCIONES,MV.TOTAL
FROM 
	...
	SUM(CASE WHEN MEDIOPAG = 'R11' THEN VALOR ELSE 0 END) AS VENTAS_CREDITO_RNY,
	SUM(CASE WHEN MEDIOPAG = 'R13' THEN VALOR ELSE 0 END) AS TARJETA_KOMIDA_RNY,
	SUM(CASE WHEN MEDIOPAG = '97' THEN VALOR ELSE 0 END) AS CAMBIOS_DEVOLUCIONES,
    SUM(VALOR) AS TOTAL
FROM 
    MvCuadre
WHERE 
    FECING >= '2026-01-01' AND TIPODCTO IN ('R1','R2')
GROUP BY 
    DCTO, TIPODCTO
	) AS MV ON DCTO=A.NRODCTO AND MV.TIPODCTO=A.TIPODCTO
WHERE        
	(A.FECHA >='2026-01-01') AND
