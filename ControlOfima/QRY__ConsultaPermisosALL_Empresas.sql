...
select * from MVCLASIFICACION WHERE  CODCLASIFICA='MTCAJAREG'
...
SELECT CODUSUARIO,MCR.NOMBRE
        FROM MVCLASIFICACION INNER JOIN
		SUPERTODO.dbo.MtCajaReg MCR ON MCR.CODIGO=MVCLASIFICACION.VALORCLASIFICA
        WHERE CODCLASIFICA = 'MTCAJAREG'
          AND CODEMPRESA IN ('SUPERTODO','BESTPRICE')
        GROUP BY CODUSUARIO,VALORCLASIFICA,MCR.NOMBRE
        HAVING COUNT(CODUSUARIO) <= 2

USE CONTROL_OFIMAEnterprise
select * from SUPERTODO.dbo.MtCajaReg
select CODCLASIFICA,CODUSUARIO,CODEMPRESA ,VALORCLASIFICA from MVCLASIFICACION WHERE CODCLASIFICA='MTCAJAREG' AND CODEMPRESA IN ('SUPERTODO','BESTPRICE') ORDER BY CODUSUARIO
...
SELECT
    CODUSUARIO,MAX(MCR.CODIGO) AS CAJA,
   ...
   1-MAX(CASE WHEN VALORCLASIFICA = 'B04' THEN 1 ELSE 0 END) AS B04,
	CODEMPRESA
FROM MVCLASIFICACION 
LEFT JOIN SUPERTODO.dbo.MtCajaReg MCR ON MCR.CODIGO=MVCLASIFICACION.VALORCLASIFICA
...
select * from MVCLASIFICACION where CODUSUARIO='JOSEA'
--QUITAR PERMISOS
--delete from MVCLASIFICACION WHERE CODUSUARIO='JUANS'

--3 QUERY COMPARA SI EXISTE O NO CLIENTExDEFECTO ======================
USE CONTROL_OFIMAEnterprise
SELECT
    CODUSUARIO,MTC.NOMBRE AS CAJA,
	CASE
WHEN EXISTS (SELECT 1 FROM SUPERTODO.dbo.MVGLOUSR MG WHERE MG.CODUSUARIO = MVCLASIFICACION.CODUSUARIO)
THEN 'EXISTE'
ELSE '4444444444'
END AS CLIENTExDEFECTO,
   1-MAX(CASE WHEN VALORCLASIFICA = '0'  THEN 1 ELSE 0 END) AS VARIOS,
   1-MAX(CASE WHEN VALORCLASIFICA = '01' THEN 1 ELSE 0 END) AS EFECTIVO,
   1-MAX(CASE WHEN VALORCLASIFICA = '03' THEN 1 ELSE 0 END) AS TAR_DEBITO,
   1-MAX(CASE WHEN VALORCLASIFICA = '04' THEN 1 ELSE 0 END) AS TAR_CREDITO,
...
FROM MVCLASIFICACION 
INNER JOIN SUPERTODO.dbo.MtCajaReg MTC ON VALORCLASIFICA=MTC.CODIGO AND MTC.CODIGO <> '0' 
WHERE CODCLASIFICA IN ('MTMEDPAG','CONSECUT','MTCAJAREG') AND CODEMPRESA IN ('SUPERTODO','BESTPRICE') 
  AND CODUSUARIO IN (
        SELECT CODUSUARIO
        FROM MVCLASIFICACION
